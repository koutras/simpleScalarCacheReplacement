...
#ifdef DEBUG
	#undef DEBUG //by a.k comment this line to enable debugging
#endif
	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  for (bindex=0,i=0; i<nsets; i++)
      ...
      if (cp->hsize)
	{
     ...
     // cp->sets[i].blks = CACHE_BINDEX(cp, cp->data, bindex); //by a.k I dont want to use random replacement 
     													//I change the order of blocks, so they are not continuous anymore... 
	...
	if(cp->policy==PLRU || cp->policy==IPV){
		/* by a.k init plru tree for each set*/
		printf("inside hot\n");
		struct cache_blk_t *ptr;
		for(j=0;j<cp->nsets;j++){

			 	i=0;
				ptr=cp->sets[j].way_head; 
				cp->sets[j].cacheLeafs[0]=ptr;
				do{
					i++;
					ptr=ptr->way_next;
					cp->sets[j].cacheLeafs[i]=ptr;
				}while(ptr!=cp->sets[j].way_tail);
			  //now the leafs contains the ways of the associative cache
				// for the current set 
			 assert(i==15); //if it's not some error happened

			if(cp->policy==PLRU)
			 for(i=0;i<TREENODES;i++) //is it correct or do I have to add 1's also?
				  cp->sets[j].prluTree[i]=isRightChild(i);


			if(cp->policy==PLRU)
				printPRLUTree(&(cp->sets[j]));
			printTableLeafs(&(cp->sets[j]));
...
cache_char2policy(char c)		/* replacement policy as a char */
{
  ...
  case 'f': return FIFO;
  case 'i': return IPV; //by a.k
  case 'p': return PLRU; // by a.k
...
//  if (CACHE_TAGSET(cp, addr) == cp->last_tagset)
 //   {
  ...
  //    blk = cp->last_blk;
   //   goto cache_fast_hit;
   // }  //by a.k disable fast hit
    
  if (cp->hsize)
    {
      /* higly-associativity cache, access through the per-set hash tables */
		assert(0); //by a.k I don't want to use the hash!
  ...
  case Random:
    ...
    break;
  case PLRU: /* by a.k miss */
  				repl=find_plru(&(cp->sets[set]));
            int lru_index=getIndex(&(cp->sets[set]),repl);
				#ifdef DEBUG
            printf("evicting node on pos %d\n",lru_index);
				#endif 
            promote(&(cp->sets[set]),repl);
				printTableLeafs(&(cp->sets[set]));
				break;
	case IPV:
				#ifdef DEBUG
				printf("cache miss inserting..\n");
				#endif
				repl=shiftBlocks(&(cp->sets[set]),15,ipv[16]); //pos 16 contains the position of a new block
				#ifdef DEBUG
				printTableLeafs(&(cp->sets[set]));
				#endif 
			  break;
  ...
  /* update block tags */
	#ifdef DEBUG
	printf("assigning tag\n");
	#endif 
  repl->tag = tag;

	#ifdef DEBUG
		if(cp->policy==IPV){
		printf("tags after assignment\n");
			printTableLeafs(&(cp->sets[set]));
	}
	#endif 
 ...
 cache_hit: /* slow hit handler */
  ...
  if (blk->way_prev && cp->policy == LRU)
	...
	/* in hit */
	if(cp->policy == PLRU){
		promote(&(cp->sets[set]),blk);
		#ifdef DEBUG
		printf("after hit!.....\n");
		printf("--------------------\n");
		printPRLUTree(&(cp->sets[set]));
		printTableLeafs(&(cp->sets[set]));
		printf("\n\n");
		#endif
	}
	if(cp->policy ==IPV){
		int index=getIndex(&(cp->sets[set]),blk);
		shiftBlocks(&(cp->sets[set]),index,ipv[index]);	
		#ifdef DEBUG
		printf("after hit!.....\n");
		printTableLeafs(&(cp->sets[set]));
		printf("\n\n");
...
/* print the contents of the 16 position buffer */
    ...
    for(i=0;i<16;i++){
        ...
        #ifdef DEBUG
        printf("[%d:%d]->",i,(int)aSet->cacheLeafs[i]->tag);
        #endif
    }
        #ifdef DEBUG
        printf("_\n");
...
void  promote(struct cache_set_t *aSet,struct cache_blk_t *p){
    ...
    #ifdef DEBUG
    printf("Promoting index %d with value \n",x);
    #endif
    while(!isRoot(x)){
        parent=getParent(x);
        #ifdef DEBUG 
        ...
        #endif
        aSet->prluTree[parent]=!isRightChild(x); //if it is a  right child make parents 
...
//after shiftBlocks, the block where the new value will be inserted
//is returned
//can be used in initial place with ipv[16] denoting the insertion place
// as well as moving a value from a position to another promoted
struct cache_blk_t *shiftBlocks(struct cache_set_t *aSet,int oldPos, int newPos){
    int i;
		struct cache_blk_t *ptr;
		struct cache_blk_t *evicted;
    assert(newPos<=oldPos);
    #ifdef DEBUG
    printf("shift blocks: shifting down from %d to %d \n",newPos,oldPos);
    #endif

	
	evicted = aSet->cacheLeafs[oldPos];
	/* switch blocks between oldPos and newPos */
   for(i=oldPos;(i>newPos)&&(i>=1);i--){
			#ifdef DEBUG
			printf("switching %d with %d \n",i,i-1);
			#endif 
			aSet->cacheLeafs[i]=aSet->cacheLeafs[i-1];
   }
	aSet->cacheLeafs[newPos]=evicted;

	//reconnect list
	ptr=aSet->way_head=aSet->cacheLeafs[0];
	ptr->way_prev=NULL;
	aSet->way_tail=aSet->cacheLeafs[15];
	aSet->way_tail->way_next=NULL;
	for(i=0;i<15;i++){
		aSet->cacheLeafs[i]->way_next=aSet->cacheLeafs[i+1];
		aSet->cacheLeafs[i+1]->way_prev=aSet->cacheLeafs[i+1];
	}
 	return evicted;
...
void switchBlocks(struct cache_blk_t *source,struct  cache_blk_t *target)
{
	struct cache_blk_t *temp_next;
	struct cache_blk_t *temp_prev;

	temp_next=source->way_next;
	temp_prev=source->way_prev;

	source->way_prev=target->way_prev;
	source->way_next=target->way_next;

	target->way_prev=temp_prev;
	target->way_next=temp_next;


	
	//the order in the tree leafs has changed, rearrange them
}




/* not used but needed for reference */
/*struct cache_blk_t *checkAndInsert(int x){
    int lru_index;
    int pos;
    cache_blk_t *blkPtr;
    ...
    if(!blkPtr){ //value not found 
        ...
        if(strategy_type==PLRU){
            ...
            shiftBlocks(15, ipv[16]); //17 position denodes the position a value is placed
        }
    }
    else{
        #ifdef DEBUG
