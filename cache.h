...
#define CACHE_HIGHLY_ASSOC(cp)	((cp)->assoc > 20) //by a.k, i think thats needed to avoid hashes and complexity
...
enum cache_policy {
  ...
  IPV,	   /* by a.k vector based promotion and insertion */
  PLRU	   /* by a.k pseudo last recently used with binary tree */
};
...
struct cache_set_t
{
  ...
  /* by a.k */
  //each way contains a tree
  int prluTree[TREENODES]; /* full tree leading to 16 associative cache */
  struct cache_blk_t *cacheLeafs[16]; //thats the main cache line, we consider that
